...
function draw(curso, chart) {
    ...
    d3.json("dados_disciplinas/" + curso + ".json", function (error, apiresponse) {
        ...
        function ticked() {
            ...
            node.attr("cx", function(d) { return Math.max(10, Math.min(width - 10, d.x)); })
               .attr("cy", function(d) { return Math.max(10, Math.min(height - 10, d.y)); });

            link.attr("x1", function(d) { return Math.max(10, Math.min(width - 10, d.source.x)); })
                .attr("y1", function(d) { return Math.max(10, Math.min(width - 10, d.source.y));; })
                .attr("x2", function(d) { return Math.max(10, Math.min(width - 10, d.target.x)); })
                .attr("y2", function(d) { return Math.max(10, Math.min(width - 10, d.target.y)); });
...
draw("administracao_d_cg", "chart1");
